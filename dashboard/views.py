...
def dashboard(request):
    ...
    if user_type == "Institute":
        ...
        student_count = len(AddStudentInst.objects.filter(
            institute=inst).values_list('student').distinct())
...
def changePassword(request):
    ...
    if request.session.get('type') == "Institute" or request.session.get('type') == "Teacher" or request.session.get('type') == "Student":
        ...
        if request.method == "POST":
            ...
            elif(len(newPassword) < 3 or len(newPassword) > 20):
                messages.warning(
...
def updateClasses(request):
    teacher = Teacher.objects.get(
        user=User.objects.get(email=request.user))
    classList, courseList, teachList, feesList = mapClassCourse(request.POST.getlist(
        'class'), request.POST.getlist('course'), request.POST.getlist('teach'), request.POST.getlist('fees'))
...
def updateDocs(request):
    teacher = Teacher.objects.get(
        user=User.objects.get(email=request.user))
    tutorid = Tutorid.objects.get(
        teacherid=teacher.id, teachername=teacher.user.username)
...
def mapClassCourse(classList, courseList, teachList, feesList):
    ...
    for i in range(len(combinedList)):
        if combinedList[i][0] == 'Other':
            ...
            for j in range(len(five_subj)):
                Nursery.append(
                    (combinedList[i][0], five_subj[j], combinedList[i][2], combinedList[i][3]))
    ...
    classList, courseList, teachList, feesList = list(
        zip(*(NurseryList + combinedList + OtherList)))
    classList, courseList, teachList, feesList = ','.join(classList), ','.join(
        courseList), ','.join(teachList), ','.join(feesList)
...
def signupTutorContinued(request, id):
    if request.session.get('type') == "Teacher":
        if request.method == 'POST':
            ...
            tutorid, created = Tutorid.objects.get_or_create(
                teacherid=id, teachername=teacher.user.username)
            tutorid.panaadhar = request.POST.get('idcard')
            tutorid.panaadharnumber = request.POST.get('idnum')
