        ...
        if request.session['type'] == "Institute":
            ...
            if request.method == "POST":
                ...
                if Exam.objects.filter(Name=name):
                    messages.warning(
                        request, "Exam With Same Name Already Exists")
        ...
        if Exam.objects.filter(institute=inst).exists():
            ...
            try:
                ...
                match_time = re.search(
                    r'\d{2}:\d{2}:\d{2}', json_datetime['datetime'])
                datetime_obj = datetime.strptime(
                    match_date.group()+' '+match_time.group(), '%Y-%m-%d %H:%M:%S')
        ...
        if courses:
            if forclass:
                teach = TeachingType.objects.filter(
                    course=courses[0], forclass=forclass).values_list('teachType')
                for i in teach:
...
def FindFees(request):
    ...
    if course:
        ...
        if courses:
            if forclass:
                fees = AddFeesC.objects.filter(
                    course=courses[0], forclass=forclass, teachType=teach)
                if fees:
        ...
        if request.session['type'] == "Institute":
            ...
            if request.session['type'] == "Teacher":
                ...
                for i in instTutor.values_list('courseName', 'forclass'):  # added
                    courses.append(Courses.objects.get(
                        courseName=i[0], forclass=i[1]))
            ...
            if exam.institute.id in instituteslist:
                if exam.course in courses:
                    for ins in instTutor:
                        batch += BatchTiming.objects.filter(
                ...
                if exam.institute.id in instituteslist:
                    if exam.course in courses:
                        if exam.status:
                            exam.status = False
                            messages.warning(
                                request, "Exam Deactivated Successfully")
                        else:
                            exam.status = True
                            messages.success(
                                request, "Exam Activated Successfully")
                        exam.save()
                    else:
                        return HttpResponse("You Are not Authenticated User for this Action")
                return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
...
def CreateQuestions(request, exam_id):
    if request.session['type'] == "Institute" or request.session['type'] == "Teacher":
        ...
        if exam.status:
            if((datetime.combine(exam.exam_date, exam.exam_time) <= datetime_obj)):
                messages.warning(
                    request, "Cannot Add Questions When Exam is Live")
                return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
                        ...
                        if options:
                            for option in options:
                                answer = MultipleAnswer(
                                    question=TutorMultipleQuestion.objects.get(
                                        id=data.id),
                                    option=option
                                )
                                answer.save()
                            ...
                            messages.warning(
                                request, 'options Cannot be Empty')
                    else:
                        options = request.POST.getlist('options', '')
...
def EditQuestions(request, exam_id):
    if request.session['type'] == "Institute" or request.session['type'] == "Teacher":
        ...
        if exam.status:
            if((datetime.combine(exam.exam_date, exam.exam_time) <= datetime_obj)):
                messages.warning(
                    request, "Cannot Edit Questions When Exam is Live")
                return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
        ...
        if request.session['type'] == "Institute":
            inst = Institute.objects.get(user=user)
            if exam.institute == inst:
                shortquestions = ShortAnswerQuestion.objects.filter(
                    exam=exam_id)
                booleanquestions = BooleanQuestion.objects.filter(exam=exam_id)
                longquestions = LongAnswerQuestion.objects.filter(exam=exam_id)
                multiplequestions = MultipleQuestion.objects.filter(
                    exam=exam_id)
                context = {
                    'exam': exam,
                    'SectionA': [],
            ...
            if enrollTutors.objects.filter(teacher=tutor).exists():
                if enrollTutors.objects.filter(teacher=tutor).exists():
                    ...
                    instTutor = enrollTutors.objects.filter(
                        teacher=tutor)  # added
                    instituteslist = []  # added
                    for i in instTutor.values_list('institute'):  # added
                        instituteslist.append(i[0])
                ...
                if exam.institute.id in instituteslist:
                    shortquestions = ShortAnswerQuestion.objects.filter(
                        exam=exam_id)
                    booleanquestions = BooleanQuestion.objects.filter(
                        exam=exam_id)
                    longquestions = LongAnswerQuestion.objects.filter(
                        exam=exam_id)
                    multiplequestions = MultipleQuestion.objects.filter(
                        exam=exam_id)
                    context = {
    ...
    if AddStudentInst.objects.filter(student=student).exists():
        ...
        if institute:
            ...
            for i in exams:
                ...
                if i.reexam_date:
                    re_exam = datetime.combine(
                        i.reexam_date, datetime.max.time())

                else:
                    re_exam = datetime.combine(
                        i.exam_date, i.exam_time) + dt.timedelta(days=3)
...
def start_exam(request, pk):
    if request.session['type'] == "Student":
        ...
        if exam_status == 'start':
            currentsession = {}
            for q in result:
                new = StudentAnswer.objects.filter(
                    student=s[0], exam=exam_mapping, question=q['question']).delete()
                new = StudentAnswer()
                new.student = s[0]
        ...
        if ans.qtype == 'multiple' or ans.qtype == 'tof':
            if ans.input_ans != 'Not Answered':
                if ans.input_ans == re.sub(re.compile('<.*?>'), '', ans.correct_ans):
                    ...
                    ans.check = 'incorrect'
                ans.save()
    StudentExamResult.objects.get_or_create(
        exam=exam, student=s, attempted=True)
    ...
    if request.session['type'] == "Teacher":
        ...
        if enrollTutors.objects.filter(teacher=tutor).exists():  # added
            ...
            for x in enrollTutors.objects.filter(teacher=tutor):
                forclass += Courses.objects.filter(
                    intitute=x.institute).values_list('forclass').distinct()
            ...
            for i, j in courses:  # added
                ...
                for ins in INSTtutor:
                    exam = Exam.objects.filter(
                        Q(institute=ins.institute) & Q(course=course))
                    exams.extend(exam)
        ...
        if exam.tutor == tutor:
            if request.method == "POST":
                ...
                try:
                    ...
                    messages.warning(request, 'Question Already Exists')
            shortquestions = TutorShortAnswerQuestion.objects.filter(
                exam=exam_id)
            booleanquestions = TutorBooleanQuestion.objects.filter(
                exam=exam_id)
            longquestions = TutorLongAnswerQuestion.objects.filter(
                exam=exam_id)
            multiplequestions = TutorMultipleQuestion.objects.filter(
                exam=exam_id)
            context = {
                'exam': exam,
                'SectionA': [],
...
def tutor_start_exam(request, pk):
    if request.session['type'] == "Student":
        ...
        if exam_status == 'start':
            currentsession = {}
            for q in result:
                new = TutorStudentAnswer.objects.filter(
                    student=s[0], exam=exam_mapping, question=q['question']).delete()
        ...
        if ans.qtype == 'multiple' or ans.qtype == 'tof':
            if ans.input_ans != 'Not Answered':
                if ans.input_ans == re.sub(re.compile('<.*?>'), '', ans.correct_ans):
                    ...
                    ans.check = 'incorrect'
                ans.save()
    TutorStudentExamResult.objects.get_or_create(
        exam=exam, student=s, attempted=True)
    del request.session['exam_status']
    return render(request, 'Exam/submitted.html')
