        ...
        if request.method == "POST":
            ...
            if(errors):
                ...
                for x in range(len(ttn)):
                    addTeacher = enrollTutors.objects.get_or_create(
                        forclass=cn[x], teachType=ttn[x], courseName=ctn[x], institute=inst, teacher=teacher)[0]
...
def viewTutors(request):
    if request.session['type'] == "Institute":
        ...
        for tut in tutors:
            courses.append(','.join([x[0] for x in enrollTutors.objects.filter(
                institute=inst, archieved=False, teacher=tut).values_list('courseName').distinct()]))
            forclass.append(','.join([x[0] for x in enrollTutors.objects.filter(
                institute=inst, archieved=False, teacher=tut).values_list('forclass').distinct()]))
            teachType.append(','.join([x[0] for x in enrollTutors.objects.filter(
                institute=inst, archieved=False, teacher=tut).values_list('teachType').distinct()]))
            availab = ','.join([x[0] for x in enrollTutors.objects.filter(
                institute=inst, archieved=False, teacher=tut).values_list('availability').distinct()])
...
def archiveTutorList(request):
    if request.session['type'] == "Institute":
        ...
        for tut in tutors:
            courses.append(','.join([x[0] for x in enrollTutors.objects.filter(
                institute=inst, archieved=True, teacher=tut).values_list('courseName').distinct()]))
            forclass.append(','.join([x[0] for x in enrollTutors.objects.filter(
                institute=inst, archieved=True, teacher=tut).values_list('forclass').distinct()]))
            teachType.append(','.join([x[0] for x in enrollTutors.objects.filter(
                institute=inst, archieved=True, teacher=tut).values_list('teachType').distinct()]))
            availab = ','.join([x[0] for x in enrollTutors.objects.filter(
                institute=inst, archieved=True, teacher=tut).values_list('availability').distinct()])
            availability.append(','.join(set(availab.split(','))))
        ...
        if request.method == "POST":
            ...
            for x in check:
                for arTutor in enrollTutors.objects.filter(teacher=Teacher.objects.get(id=x)):
                    ...
                    arTutor.save()
            messages.success(
                request, "Teacher Removed from Archieve Successfully")
            return redirect('viewTutors')
...
def createReccurance(utcDateTime, duration, recc, pattern, repeat, days, utcEndingdate):
    ...
    if(recc):
        if(pattern == 'D'):
            days = allDays
            repeat = 1
        dayConv = {'Mon': rrule.MO, 'Tue': rrule.TU, 'Wed': rrule.WE,
                   'Thu': rrule.TH, 'Fri': rrule.FR, 'Sat': rrule.SA, 'Sun': rrule.SU}
        daysDump = [utcDateTime]+list(rrule.rrule(rrule.WEEKLY, interval=repeat, wkst=rrule.MO, byweekday=[
                                      dayConv[x] for x in days], until=utcEndingdate, dtstart=utcDateTime))
    else:
...
def teaMakeAppointment(request, id):
    if request.session['type'] == "Teacher":
        ...
        if request.method == 'POST':
            ...
            try:
                duration = timedelta(hours=durationHour,
            ...
            try:
                utcDateTime = old_timezone.localize(
                    dateTimeObj).astimezone(new_timezone)
            except:
            ...
            if(endingdate):
                try:
                    ...
                    utcEndingdate = old_timezone.localize(
                        endingdate).astimezone(new_timezone)
                except Exception as e:
                    errors.append(f"Date is in wrong format > {date}")
            else:
            ...
            daysDump = createReccurance(
                utcDateTime, duration, recc, pattern, repeat, days, utcEndingdate)
            # checking clashes for student
            try:
                if(checkClashes(tutor, utcDateTime, duration, recc, pattern, repeat, days, utcEndingdate, daysDump)):
                    errors.append(
                        "This appontment cannot be created as you already have an appointment during this duration.")
            except Exception as e:
                errors.append(
                    "There was an error while creating this appointment for you.")

            # checking clashes for tutor
            try:
                if(checkClashes(student, utcDateTime, duration, recc, pattern, repeat, days, utcEndingdate, daysDump)):
                    errors.append(
                        "This appontment cannot be created as this student already have an appointment during this duration.")
            except Exception as e:
                errors.append(
                    "There was an error whicle creating this appointment for the tutor.")
...
def viewAssignmentTutor(request):
    if request.session['type'] == "Teacher":
        ...
        try:
            json_datetime = requests.get(
                'http://worldtimeapi.org/api/ip', timeout=10)
            json_datetime = json.loads(json_datetime.content)
            match_date = re.search(
                r'\d{4}-\d{2}-\d{2}', json_datetime['datetime'])
            datetime_obj = datetime.strptime(match_date.group(), '%Y-%m-%d')

        except:
            datetime_obj = dt.date.today()

        initial = PostAssignment.objects.filter(
            deadline__gte=datetime_obj, assigned=False)
        ...
        for i in range(len(class_list)):
            currentS.extend(initial.filter(
                forclass=class_list[i], courseName=course_list[i]))
        ...
        if request.method == "POST":
            ...
            if address:
                currentS = currentS.filter(
                    Q(student__address__icontains=address))
            if budgetVal:
