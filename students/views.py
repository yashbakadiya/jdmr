...
def viewStudents(request):
    if request.session["type"] == "Institute":
        ...
        for student in students:
            courses.append(','.join([x[0] for x in AddStudentInst.objects.filter(
                institute=inst, archieved=False, student=student).values_list('courseName').distinct()]))
            forclass.append(','.join([x[0] for x in AddStudentInst.objects.filter(
                institute=inst, archieved=False, student=student).values_list('forclass').distinct()]))
            teachType.append(','.join([x[0] for x in AddStudentInst.objects.filter(
                institute=inst, archieved=False, student=student).values_list('teachType').distinct()]))
...
def archiveStudentList(request):
    if request.session['type'] == "Institute":
        ...
        for student in students:
            courses.append(','.join([x[0] for x in AddStudentInst.objects.filter(
                institute=inst, archieved=True, student=student).values_list('courseName').distinct()]))
            forclass.append(','.join([x[0] for x in AddStudentInst.objects.filter(
                institute=inst, archieved=True, student=student).values_list('forclass').distinct()]))
            teachType.append(','.join([x[0] for x in AddStudentInst.objects.filter(
                institute=inst, archieved=True, student=student).values_list('teachType').distinct()]))
        ...
        if request.method == "POST":
            ...
            for x in check:
                for arStudent in AddStudentInst.objects.filter(student=Student.objects.get(id=int(x))):
                    ...
                    arStudent.save()
            messages.success(
                request, "Student Removed from Archieve Successfully")
            return redirect('viewStudents')
...
def postAssignment(request):
    if request.session['type'] == "Student":
        ...
        if request.method == 'POST':
            ...
            if deadline:
                postAssigObj.deadline = datetime.strptime(
                    deadline, '%Y-%m-%d').date()
            postAssigObj.save()
        ...
        if request.method == 'POST':
            ...
            try:
                if(checkClashes(tutor, utcDateTime, duration, recc, pattern, repeat, days, utcEndingdate, daysDump)):
                    errors.append(
                        "This appontment cannot be created as this tutor already have an appointment during this duration.")
            except Exception as e:
                errors.append(
                    "There was an error whicle creating this appointment for the student.")
...
def enrolledStudents(request):
    if request.session['type'] == "Teacher":
        ...
        for i in range(len(class_list)):
            currentS.extend(PostTution.objects.filter(
                forclass=class_list[i], subject=course_list[i]))
