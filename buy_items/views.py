...
def revenue(request):
    ...
    date = datetime.strptime(match_date.group()+' '+match_time.group(), '%Y-%m-%d %H:%M:%S')

    price = request.session['price']
    product = request.session['product']
    category = request.session['category']

    Revenue.objects.create(owner=owner,customer=customer,date=date,price=price,product=product,category=category)
    ...
    if daily_earnings:
        ...
        for year in range(int(start_year),int(end_year)+1):
            while month<13:
                ...
                monthly = daily_earnings.filter(date__year=year,date__month=month)
                if monthly:
                    monthly_earnings.append([monthly[0].date,monthly.aggregate(Sum('price'))])
