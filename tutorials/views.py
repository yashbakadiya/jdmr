        ...
        if request.method == "POST":
            ...
            data = TutorialInstitute(
                ...
                Course=Courses.objects.filter(
                    intitute=inst, courseName=course, forclass=forclass).first(),
                forclass=forclass,
                Fees=fees,
                Duration=duration,
        ...
        if request.method == "POST":
            ...
            for i in ids:
                if TutorialInstitute.objects.filter(id=i).exists():
                    ...
                    tutorial.save()
            messages.success(
                request, 'Tutorial Removed from Archive Successfully')
            return redirect('viewtutorials')
...
def EditTutorialsInstitute(request, course_id):
    if request.session['type'] == "Institute":
        ...
        if request.method == "POST":
            ...
            if course:
                tutorial.Course = Courses.objects.filter(
                    intitute=inst, courseName=course, forclass=forclass).first()
            if feeDisc:
...
def DeleteTutorialsInstituteVideos(request, playlist_id):
    if request.session['type'] == "Institute":
        ...
        data.delete()
        count = TutorialInstitutePlaylist.objects.filter(
...
def DeleteTutorialsTutorVideos(request, playlist_id):
    if request.session['type'] == "Teacher":
        ...
        tutorial = data.tutorial
        data.delete()
        count = TutorialTutorsPlaylist.objects.filter(
            tutorial=tutorial).count()
        if count < 1:
...
def SearchCourses(request):
    ...
    if request.session['type'] == "Student":
        ...
        if request.method == 'POST':
            ...
            buyInstituteTutorial = BuyInstituteTutorial.objects.filter(
                student=student)
            buyInstituteTutorialList = [
                buy.tutorial.id for buy in buyInstituteTutorial]
            boughtInstituteTutorial = tutorials_ins_item.filter(
                id__in=buyInstituteTutorialList)
            # not bought intitute tutorial objects
            notBoughtInstituteTutorial = tutorials_ins_item.exclude(
                id__in=buyInstituteTutorialList)

            # Tutor
            # bought tutor tutorial objects
            buyTutorTutorial = BuyTutorTutorial.objects.filter(student=student)
            buyTutorTutorialList = [
                buy.tutorial.id for buy in buyTutorTutorial]
            boughtTutorTutorial = tutorials_tutor_item.filter(
                id__in=buyTutorTutorialList)
            # not bought intitute tutorial objects
            notBoughtTutorTutorial = tutorials_tutor_item.exclude(
                id__in=buyTutorTutorialList)
            bought = chain(boughtInstituteTutorial, boughtTutorTutorial)
            notBought = chain(notBoughtInstituteTutorial,
                              notBoughtTutorTutorial)
